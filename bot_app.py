...
class BotApp:
    ...
    def _hotkey_add_wait_step(self):
        self.root.after(0, self.add_wait_color_step)
    ...
    def add_wait_color_step(self):
        """
        F4'e basildiginda calisir. Kullaniciya 'fareni hedefin ustune goturup
        bekle' diye 3 saniyelik bir geri sayim gosterir. Sayac bitince, fare
        konumundaki PIKSEL RENGINI okur ve kucuk bir onay penceresinde gosterir.
        Kullanici 'Evet, dogru' derse bu renk hedef olarak kaydedilir.
        ONEMLI: bu adimi eklerken uyari/buton EKRANDA GORUNUR olmali, cunku
        okunan renk o butonun/uyarinin rengi olacak.
        """
        ...
        self.root.after(3100, lambda: self._read_color_at_mouse_position(countdown_win))

    def _read_color_at_mouse_position(self, countdown_win):
        countdown_win.close()
        x, y = pyautogui.position()
        color = get_pixel_color(x, y)

        confirmed = ColorConfirmDialog(self.root, x, y, color).show()
        if not confirmed:
            self.set_status("Bekleme/Onay adimi iptal edildi.")
            return
        ...
        xr = x / self.recorder.screen_w
        yr = y / self.recorder.screen_h
...
class ColorConfirmDialog:
    """
    F4 ile fare konumundan okunan rengi kucuk bir renk kutusu ile gosterip
    kullanicidan onay ister: 'Bu renk dogru mu?' Evet/Hayir.
    """
    ...
    def show(self):
        ...
        ttk.Label(frame, text=f"Yakalanan nokta: ({self.x}, {self.y})",
                  font=("Segoe UI", 9)).pack(anchor="w")
        ttk.Label(frame, text=f"Renk (RGB): {self.color}",
                  font=("Segoe UI", 9)).pack(anchor="w", pady=(2, 8))

        swatch = tk.Canvas(frame, width=80, height=40, bg=hex_color,
                            highlightthickness=1, highlightbackground="#888")
        swatch.pack(pady=(0, 10))

        ttk.Label(frame, text="Bu renk hedef butonun/uyarinin rengi mi?",
        ...
        ttk.Button(btn_row, text="Evet, dogru", command=on_yes).pack(side="left", padx=4)
        ttk.Button(btn_row, text="Hayir, iptal et", command=on_no).pack(side="left", padx=4)

        top.protocol("WM_DELETE_WINDOW", on_no)

        # Pencereyi ekran ortasina yerlestir
